...
function changeCount( num,fid  ){
	$.ajax({
		...
		success:function(data){
			if(   data.code==1 ){
				//   obj   ->   total:  xxx      cartItem( resfood, count,smallcount)
				if(  data.obj.cartItem.count<=0  ){
					//TODO: 从cart表格删除    id 为   tr_fid 的行
					//$("#cart").remove($("#tr_"+fid));
					$("#tr_"+fid).remove(  );
				}else{
			...
			<c:forEach items="${sessionCart }" var="entry">
					<tr bgcolor="#ffffff" id="tr_${entry.key }">
								<td width="50" align="center" height="22">
										<font   color="#000000">${entry.key }   </font>
								</td>
								<td align="center" height="22">
								      	<font color="#000000">${entry.value.resfood.fname }</font>
										<input type="hidden" name="fid" value="${entry.key }">
								</td>
								<td width="104" align="center" height="22">
										<font 	color="#000000">￥${entry.value.resfood.realprice }</font>
							    </td>
				...
				<table width="300" border="0" cellspacing="1" cellpadding="4"
					...
					<tr bgcolor="#dadada">
								...
								<font color="#000000"><a href="index.jsp">继续购物</a></font></td>
								<td height="10" align="center" style="cursor:hand" onClick="">
								<font 	color="#000000"><a href="resuser/resorder.action?op=toFillForm">生成订单</a></font></td>
					</tr>
				</table>
				</td>
			</tr>
